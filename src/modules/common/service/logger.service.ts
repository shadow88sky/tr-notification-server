 ...
 */
 ...
 const logFormatter = printf((options: FormatOptions) => {
     options.message =
         options.error && options.error.stack
             ? options.error.stack
             : options.message;
     const msg = `{"time":"${options.timestamp}","level":"${options.level
 ...
 export class LoggerService {
     ...
     info(message: string , options?: IOptions) {
         const meta = {
             module: options && options.module || GetCallerModule(2).callSite.getFileName(),
     ...
     error(message: Error, options?: IOptions) {
         ...
         this.defaultLog.error(JSON.stringify(message.message) + message.stack, { options, ...meta });
     }
